...
// here we again are passing one argument that then gets applied map with our already defined transform function transform
,nevermind that we do not have transform(argument) it will do transform with whatever is currently in iteration of map function 
,because thats just how map() works.

// This is what .map() does internally
Array.prototype.map = function(callbackFn) { 
  // 'callbackFn' is the name 'map' gives to your 'transform' function
...
// and map function iterates through array and for every element it applies function passed in argument of map(here) 
then returns NEW array back without mutating old one

// this is sort of composition, we pass one argument, in this case array it would fail if it wasnt array 
    // if we passed number to map, in this case processNumbars(123)
    // JS tries to find .map of number 123 
    // it cant find it
