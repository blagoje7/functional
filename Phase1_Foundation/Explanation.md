...
- `The problem` : If a variable x is modified by function A and function B also uses x. Result of function B is entirely dependant on when is function B called relative to function A.
- `The trap` : Non-determinism, you cannot test function B in isolation
...
- `The problem` : In imperative code functions often do secret work, writing to a database, changing DOM, logging to the console...
...
- `Concept` : This breaks Referential Transparency ( ability to replace a function call with just its result without breaking the program )
# 3. Complexity overhead
- `Imperative` : describes "how" to do something ( step by step instructions "Iterate from 0 to 10; check if even; push to array )
- `Functional` : describes "what" something is ( declarative "The result is a filtered list of evens" )
...
# 2. The hidden input 
Function accessing anything but its arguments
